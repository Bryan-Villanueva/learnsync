    ...
    filteredItems.forEach(missed => {
      ...
      missedCard.innerHTML = `
        ...
        <div style="display: flex; gap: 0.75rem; flex-wrap: wrap; padding-top: 1rem; border-top: 2px solid #f0f0f0;">
          ...
          <button onclick="rescheduleAssignment('${missed.classId}', '${missed.assignmentId}')"
                  class="btn-primary"
                  style="flex: 1; min-width: 150px; background: #fd7e14;">
...
function openClass(classId) {
  ...
  // ✅ FIX: Load fresh data from database when opening class
  loadClassDataFromDatabase(classItem).then(() => {
    //FIX: Hide ALL content sections first
    contentSections.forEach(section => {
      section.classList.remove('active');
    });
    
    //FIX: Remove active class from all nav links
    navLinks.forEach(link => {
      link.classList.remove('active');
    });
...
async function saveRescheduledDate(classId, assignmentId) {
  ...
  try {
    ...
    // Update calendar
    const dateKey = `${newDate.getFullYear()}-${newDate.getMonth() + 1}-${newDate.getDate()}`;
...
function loadClassAssignments() {
  ...
  sortedAssignments.forEach(assignment => {
    // ✅ FIX: Get fresh submission count from database
    const submissionCount = assignment.submissions ? assignment.submissions.length : 0;
...
async function saveMaterial(material) {
  ...
  try {
    ...
    const result = await response.json();
    material.id = result.material_id; // Update with DB ID
    ...
    if (localClassIndex !== -1) {
      if (!localClasses[localClassIndex].materials) {
        localClasses[localClassIndex].materials = [];
      }
      localClasses[localClassIndex].materials.push(material);
      localStorage.setItem('professor_classes', JSON.stringify(localClasses));
    }
...
function loadClassStudents() {
  ...
  sortedStudents.forEach(student => {
    ...
    // ✅ FIX: Load student's profile picture from localStorage
    const userType = 'student';
    const savedAvatar = localStorage.getItem(`${userType}_avatar_${studentDatabaseId}`);
    const avatarUrl = savedAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(studentName)}&background=4a90a4&color=fff`;
