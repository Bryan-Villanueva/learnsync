...
function checkStorageQuota() {
    if (navigator.storage && navigator.storage.estimate) {
        navigator.storage.estimate().then(estimate => {
            ...
            if (percentUsed > 90) {
                alert('⚠️ Storage almost full! Please clear old data or contact administrator.');
            }
        });
    }
}
...
// ✅ FIX: Better submit button handling with proper error recovery
...
if (submitBtn) {
  ...
  newSubmitBtn.addEventListener('click', async function(e) {
    ...
    try {
      ...
      // Success - modal will be closed by submitAssignment function
    } catch (error) {
  ...
  if (avatarUpload && profileAvatar && topbarAvatar) {
    ...
    newAvatarUpload.addEventListener('change', function(e) {
      ...
      reader.onload = function(e) {
        ...
        try {
          const userType = 'student';
          // ✅ FIX: Save with user ID so professor can access it
...
async function loadArchivedClasses() {
  try {
    ...
    if (response.ok) {
      ...
      // ✅ FIX: Load materials and assignments for archived classes
      for (let classItem of archivedClasses) {
        // Load materials from database
        try {
          const materialsResponse = await fetch(`/api/professor/classes/${classItem.id}/materials`);
          if (materialsResponse.ok) {
          ...
          console.error('Error loading archived materials:', e);
          classItem.materials = [];
        }
        ...
        try {
          ...
          console.error('Error loading archived assignments:', e);
          classItem.assignments = [];
        }
      }
...
function openClass(classId, isArchived = false) {
  ...
  if (!classItem && isArchived) {
    // ✅ FIX: Load archived class with materials and assignments
    fetch('/api/professor/classes?archived=true')
      .then(response => response.json())
      .then(async archivedClasses => {
        classItem = archivedClasses.find(c => c.id === classId);
        if (classItem) {
          // ✅ FIX: Load materials and assignments for archived class
          try {
            const materialsResponse = await fetch(`/api/professor/classes/${classItem.id}/materials`);
            if (materialsResponse.ok) {
              classItem.materials = await materialsResponse.json();
              console.log(`✅ Loaded ${classItem.materials.length} materials for archived class`);
            }
          } catch (e) {
            console.error('Error loading archived materials:', e);
            classItem.materials = [];
          ...
          try {
            ...
            if (assignmentsResponse.ok) {
              classItem.assignments = await assignmentsResponse.json();
              console.log(`✅ Loaded ${classItem.assignments.length} assignments for archived class`);
            }
          } catch (e) {
            console.error('Error loading archived assignments:', e);
            classItem.assignments = [];
...
function displayClassView(classItem, isArchived = false) {
  ...
  // ✅ FIX: Use the already loaded materials and assignments
  ...
  // ✅ FIX: Add archived indicator to class view
  if (isArchived) {
    classView.dataset.archived = 'true';
    classView.style.opacity = '0.9';
    classView.style.pointerEvents = 'none'; // Disable interactions
    ...
    if (!existingBanner) {
      ...
      banner.style.cssText = `
        ...
        border-radius: 8px;
      `;
      banner.innerHTML = '<i class="fas fa-archive"></i> This class is archived - Read Only Mode';
    ...
    classView.style.pointerEvents = 'auto';
  ...
  // ✅ FIX: Load content after data is ready
  loadClassPosts();
...
async function submitAssignment() {
  ...
  if (!assignmentId) {
    alert('❌ Error: Assignment ID not found. Please close and reopen the submission form.');
    return;
  }
  ...
  // ✅ FIX #1: HANDLE MULTIPLE FILES
  if (filesInput && filesInput.files && filesInput.files.length > 0) {
    try {
      const totalFiles = filesInput.files.length;
      console.log(`📎 Uploading ${totalFiles} file(s)...`);
      ...
      for (let i = 0; i < totalFiles; i++) {
        ...
        console.log(`⬆️ Uploading file ${i + 1}/${totalFiles}: ${file.name}`);
        
        if (submitBtn) {
          submitBtn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> Uploading ${i + 1}/${totalFiles}...`;
        }
      ...
      if (submitBtn) {
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Saving submission...';
      }
      
      console.log(`✅ All ${totalFiles} files uploaded successfully`);
...
function loadClassGrades() {
  ...
  // ✅ FIX: Load fresh assignment data with submissions
  const professorClasses = localStorage.getItem('professor_classes');
  let profClasses = [];
  if (professorClasses) {
    try {
