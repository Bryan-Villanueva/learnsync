    ...
    if request.method == 'POST':
        ...
        if reset_token.user_type == 'student':
            ...
            if (user.first_name.lower() == first_name.lower() and 
                ...
                flash("The information you provided does not match our records. Please try again.", "error")
        else:
...
def signup():
    if request.method == 'POST':
        ...
        if not policy_agreed:
            flash("You must agree to the policy!", "error")
            return redirect(url_for('signup'))
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, password):
            ...
            if attempts >= MAX_LOGIN_ATTEMPTS:
                ...
                flash(f"Too many failed attempts. Account locked for {LOGIN_TIMEOUT_MINUTES} minutes.", "error")
            else:
...
# In app.py - UPDATE the professor classes API endpoint
...
def manage_classes():
    ...
    if user_type == 'professor':
        ...
        if request.method == 'GET':
            try:
                # ✅ FIX: Properly filter by archived status
                show_archived = request.args.get('archived', 'false').lower() == 'true'
                
                # Query with explicit archived filter
                if show_archived:
        ...
        try:
            ...
            for cls in student.classes:
                # Only show classes that match the archived status filter
                if cls.archived == show_archived:
                    ...
                    classes_data.append({
                        ...
                        'description': cls.description,
                        'code': cls.code,
                        'archived': cls.archived,  # Include archived status
                        'professor_name': f"{professor.first_name} {professor.last_name}" if professor else "N/A",
                        'materials': [],
                        'assignments': []
                    })
...
def professor_stats():
    ...
    # ✅ FIX: Use set to track unique students
    unique_students = set()
...
def contact_us():
    try:
        ...
        user_email = request.form.get('email')  # User's email
        message = request.form.get('message')
        
        print(f"📧 Contact form submission: {name}, {user_email}, {message}")
        
        if not all([name, user_email, message]):
            return jsonify({'error': 'All fields are required'}), 400
        
        # Email configuration - Use YOUR email as the SMTP sender
        smtp_server = os.environ.get("SMTP_SERVER", "smtp.gmail.com")
        smtp_port = int(os.environ.get("SMTP_PORT", "587"))
        smtp_email = os.environ.get("SENDER_EMAIL", "bryanvillanueva426@gmail.com")  # Your email for SMTP
        smtp_password = os.environ.get("bzsx ubth usuw uquv")  # Your app password
        recipient_email = os.environ.get("RECIPIENT_EMAIL", "bryanzkie3@gmail.com")

        if not smtp_password:
            print("❌ SMTP password not configured")
            return jsonify({'error': 'Email service not configured properly'}), 503
        
        # Create message
        msg = MIMEMultipart()
        msg['From'] = f"{name} <{smtp_email}>"  # Your email as SMTP sender
        msg['To'] = recipient_email
        msg['Reply-To'] = user_email  # User's email for replies
        msg['Subject'] = f"LearnSync Contact: Message from {name}"
        ...
        # Send email using YOUR SMTP credentials
        ...
        text = msg.as_string()
        server.sendmail(smtp_email, recipient_email, text)  # Your email sends it
        server.quit()
        ...
        return jsonify({'error': 'Email authentication failed. Please check email configuration.'}), 500
